...
def train():
    ...
    train_df, val_df = train_test_split(df, test_size=0.2, random_state=42, stratify=df['bin'])
    ...
    train_dataset = AdvancedSkyDataset(train_df, is_train=True, mean=train_mean, std=train_std)
    val_dataset = AdvancedSkyDataset(val_df, is_train=False, mean=train_mean, std=train_std)
    ...
    scheduler = torch.optim.lr_scheduler.OneCycleLR(optimizer, max_lr=5e-5, total_steps=50*len(train_loader), pct_start=0.3)
    ...
    for stage in range(2):
        if stage == 1:
            ...
            scheduler = torch.optim.lr_scheduler.OneCycleLR(optimizer, max_lr=5e-6, total_steps=50*len(train_loader), pct_start=0.3)
        ...
        for epoch in range(50):
            ...
            for i, (imgs, labels) in enumerate(tqdm(train_loader, desc=f"Stage {stage}, Epoch {epoch}")):
                imgs, labels = imgs.to(device), labels.to(device)
                
                if torch.isnan(imgs).any() or torch.isinf(imgs).any() or torch.isnan(labels).any() or torch.isinf(labels).any():
            ...
            print(f"Stage {stage}, Epoch {epoch}: Train Loss: {train_loss:.6f}, Val RMSE: {val_rmse:.6f}")
